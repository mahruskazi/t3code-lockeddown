...
export function makeDesktopContentSecurityPolicy(input: DesktopProtocolRegistrationInput): string {
  ...
  // The renderer connects to the loopback backend this app supervises. The
  // port is not known when this response policy is created, so restrict
  // connections by scheme instead of by host.

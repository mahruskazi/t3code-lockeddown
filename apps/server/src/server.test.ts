...
const makeCloudMintCredentialRequest = (input: {
  ...
  const header = Buffer.from(
    JSON.stringify({ alg: "EdDSA", typ: "t3-relay-mint+jwt" }),
  ).toString("base64url");
...
const makeCloudEnvironmentHealthRequest = (input: {
  ...
  const header = Buffer.from(
    JSON.stringify({ alg: "EdDSA", typ: "t3-relay-health+jwt" }),
  ).toString("base64url");
  const encodedPayload = Buffer.from(JSON.stringify(payload)).toString("base64url");
  const signingInput = `${header}.${encodedPayload}`;
  return {
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("records browser OTLP trace exports locally without forwarding them", () =>
    Effect.gen(function* () {

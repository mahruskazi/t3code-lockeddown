...
export async function readTranscriptRecords(
  ...
  try {
    ...
    if (
      ...
      // [fork:pi] Pi's scan state is not carried in TranscriptParsePosition, so
      // a resumed read would lose its session id and model attribution.
      provider !== "pi" &&
    ...
    const parseLine = (line: string, state: CodexScanState, out: UsageRecord[]): void => {
      ...
      if (provider === "pi") {
        if (!piLineMightMatter(line)) return;

...
> **[fork:lockdown] Nothing on this page leaves this machine.** This is a
> hardened fork: `AnalyticsService.layer` is bound to the no-op service, so no
> PostHog client is constructed, no flush fiber runs, and no environment
> variable re-enables sending. The events below are still recorded into that
> sink, which is why the taxonomy is worth keeping accurate — but the delivery
> half of this document describes upstream, not this build. See
> [`AGENTS.md`](../../AGENTS.md#the-invariants), invariant 1.

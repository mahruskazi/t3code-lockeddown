...
`chat.new` creates a normal new thread using configured and sticky defaults; with more than one
project, it opens the project chooser first. `chat.newLocal` (⌘⇧N by default on macOS) duplicates
